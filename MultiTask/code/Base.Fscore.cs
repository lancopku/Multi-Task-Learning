...
namespace Program
{
    class fscore
    {
        public static List<double> getFscore(List<string> goldTagList, List<string> resTagList, List<double> infoList)
        {
            ...
            //reduce original tags to 3 tags: B(x), I, O
        ...
        static void getNewTagList(baseHashMap<string, string> tagMap, ref List<string> tagList)
        {
            ...
            foreach (string im in tagList)
            {
                ...
                {
                    if (!tagMap.ContainsKey(tagAry[i]))
                        throw new Exception("error");
                    tagAry[i] = tagMap[tagAry[i]];
                }
            ...
            foreach (string im in tagList)
            {
                ...
                for (int i = 0; i < tagAry.Length; i++)
                {
                    if (tagAry[i].StartsWith("B"))
                    {
                        ...
                        string chunk = type + "_" + length + "_" + pos;
                        chunks += chunk + ",";
                    }
                }
                tmpList.Add(chunks);
